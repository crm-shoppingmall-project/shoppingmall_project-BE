...
public class CartController {
    ...
    @Operation(summary = "장바구니 목록 페이징 조회", description = "로그인한 사용자의 장바구니 목록을 페이징하여 조회합니다.")
    public ResponseEntity<Page<CartDetailDto>> getCartDetailsPage(@AuthenticationPrincipal DetailsUser user,
                                                                  Pageable pageable) {
        Page<CartDetailDto> cartDetails = cartService.getCartDetailsPage(user.getMember().getMemberId().intValue(),
                pageable);
        return ResponseEntity.ok(cartDetails);
    }
